...
IMG ?= gcr.io/livewyer-ops-public/calibre-demo:v1
...
install-output:
	$(KUSTOMIZE) build config/crd > crd-manifest.yaml
...
deploy-output: manifests kustomize ## Deploy controller to the K8s cluster specified in ~/.kube/config.
	cd config/manager && $(KUSTOMIZE) edit set image controller=${IMG}
